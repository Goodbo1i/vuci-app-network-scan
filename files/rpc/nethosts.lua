...
local parser = argparse("nethosts", "Scan a network for hosts, identifying which are known and unknown")    
parser:option("-i --ipsubnet", "Specific ipv4 subnet to scan")  --GETS IP/NETMASK THAT WERE INSERTED
parser:option("-r, --range", "Port range to scan")  --GETS PORTS RANGE THAT WAS INSERTED
parser:flag("-p --ports", "Prints out open ports")
...
NetworkDatabase.KnownHosts = {
    {   macAddr = "00:1E:42:23:E0:67",
        description = "My router",
        vendor = "Teltonika RUTX_08",
    },
}
...
function sortHostsByFamiliarity ( HostsFoundOnNetwork )
    ...
    os.remove("address_ports")
    for _, ThisNetworkHost in ipairs( HostsFoundOnNetwork ) do
        -- Ensure that the host has a MAC address (will be used as a key).
        if args.ports then --CHECKS IF SCAN PORTS IS TRUE
            local cmd = "nmap -sT "
            if args.range ~= nil then --CHACKS IF PROT RANGE IS DEFINED BY USER
                cmd = cmd .. "-p " .. args.range .. " " --INPUTS PORT RANGE IN TERMINAL
            else        --IF RANGE IS NOT DEFIEND, THEN ON DEFAULT DOES FAST SCAN
                cmd = cmd .. "-F "
            end
            cmd = cmd ..ThisNetworkHost.ipNumber.." | grep 'open' | awk '{ print $1 }' | awk -F/ '{print \""..ThisNetworkHost.ipNumber.." \" $1}' >> address_ports" --TERMINAL OUTPUTS NORMAL TEXT
            os.execute(cmd)
...
function printHostReportRecord ( familiarityTag, NetworkHost )
    ...
    local vendor = NetworkHost.vendor
    if vendor == nil then --IF NO VENDOR THEN OUTPUTS UNKNOWN
        vendor = "(Unknown)"
    end
    ...
    local jsonFormat = "{\"host\":\"%s\",\"ipNumber\":\"%s\",\"macAddress\":\"%s\",\"vendor\":\"%s\",\"ports\":[%s]}" --JSON FORMAT TO INPUT INTO
    if args.ports then
        local ports = Group_ports_by_address()  --CALLS A FUNCTION TO MAKE PORT DATA VAIABLE OPTION FOR JSONFORMAT  
        local portsText = ""
        if ports[ipNumberString] ~= nil then --IF NO MORE PORTS THEN CREATING STRING FROM TABLE ELEMENTS 
            portsText = table.concat(ports[ipNumberString], ",")
        end
        io.write(string.format(jsonFormat, familiarityTag, ipNumberString, macAddrString, vendor, portsText)) --INSERTING DATA INTO JSONFORMAT 
        return
    end
    -- Use the provided format string to print this host record.
    io.write(string.format(jsonFormat, familiarityTag, ipNumberString, macAddrString, vendor, "")) --INSERTING DATA INTO JSONFORMAT 
...
function printHostReport ( Subnet, SortedHosts, familiarityTag )
    ...
    local key_count = Get_key_count(SortedHosts) --CALLS A FUNCTION TO GET COUNT OF HOSTS
    local count = 0
    io.write("[")
    for _, ThisHost in ipairs( SortedHosts ) do
        printHostReportRecord( familiarityTag, ThisHost )
        count = count + 1
        if count < key_count then io.write(",") end --INSERTS ',' AFTER EACH HOST
    end
    io.write("]")
end

function Split (inputstr, sep) --FUNCTION FOR SPLITING
    local t={}
...
function Group_ports_by_address() --FUNCTION GROUPS PORTS AND MAKES READABLE FOR JSON FORMAT
    local address_ports = {}
    for line in io.lines("address_ports") do 
        local data = Split(line, ' ')
        local address = data[1]
        local port = data[2]
        if not address_ports[address] then
            address_ports[address] = {};
        end
        table.insert(address_ports[address], port);
    end
...
function genNetworkHostsReport ( Subnet,
    ...
    io.write("{")   --ADDIDIONAL BRACKETS TO PASS JSON FORMAT
    io.write("\"known\":")  --KNOWN HOSTS
    -- Start with the known hosts, then the unknown hosts.
    printHostReport( Subnet, HostsThatAreKnown, isKnownTag )
    io.write(",")
    io.write("\"unknown\":")    --UNKNOWN HOSTS
